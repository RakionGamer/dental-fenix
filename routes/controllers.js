...
controller.delete = (req, res) => {
    const { id } = req.params;
    req.getConnection((err, conn) => {
        conn.query('DELETE FROM inventario WHERE id_product = ?', [id], (err, row) => {
            res.redirect('/');
        })
    })
}
...
controller.edit = (req, res) => {
    const { id } = req.params;
    req.getConnection((err, conn) => {
        conn.query('SELECT * FROM inventario WHERE id_product = ?', [id], (err, row) => {
            res.render('update', {
                data: row
            });
        })
...
controller.update = (req, res) => {
    ...
    req.getConnection((err, conn) => {
        conn.query('UPDATE inventario set ? WHERE id_product = ?', [product, id], (err, row) => {
            res.redirect('/');
        });
    })
}
...
controller.user = async (req, res) => {
    const data = req.body;
    req.getConnection((err, conn) => {
        conn.query("SELECT * FROM users WHERE user = ? AND password = ?", [data.user, data.password], (err, row) => {
            if (err) return err;
            if (row.length > 0) {
                const token = jwt.sign({ id: row[0].id }, 'token');
                res.cookie('jwt', token);
                res.redirect('/');
            } else {
                res.render('login', { data: true, msg: '¡Usuario o contraseña incorrectas!' });
            }
        })
    }
    )
